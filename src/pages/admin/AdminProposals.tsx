...
import { Loader2, FileText, RefreshCw, Search, X, Eye } from "lucide-react";
...
import {
  adminApi, adminSession, type CmsProposalSummaryExt,
...
export default function AdminProposals() {
  ...
  const [rows, setRows] = useState<CmsProposalSummaryExt[]>([]);
  ...
  const load = async (
    ...
    try {
      ...
      const res = await adminApi.listAllProposals({
        page: p,
        per_page: PER_PAGE,
        q: q || undefined,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="font-baskerville text-3xl text-foreground mb-2">Proposal Submissions</h1>
          <p className="text-muted-foreground text-sm">
            Book proposals submitted via the public form
            {adminUser?.email && (
      ...
      >
        <div className="lg:col-span-2">
          ...
          <div className="relative">
            ...
            <Input
              ...
              placeholder="Reference, title, author name or email"
        ...
        <div>
          ...
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            <SelectTrigger className="h-9 rounded-none"><SelectValue placeholder="All" /></SelectTrigger>
            <SelectContent>
          ...
          <table className="w-full text-sm">
            <thead className="bg-[#f4f3ec] text-left text-xs uppercase tracking-wider text-muted-foreground">
              <tr>
                ...
                <th className="px-4 py-3">Lead author</th>
                <th className="px-4 py-3 text-center">Authors</th>
            ...
            <tbody>
              {rows.map((r) => (
                <tr
                  ...
                  onClick={() => navigate(`/admin/proposal-submissions/${r.id}`)}
                >
                  <td className="px-4 py-3 font-medium">{r.reference_number}</td>
                  <td className="px-4 py-3 text-muted-foreground">{formatDate(r.submitted_at)}</td>
                  <td className="px-4 py-3">
                    <span className={`inline-block px-2 py-0.5 text-xs uppercase tracking-wider ${statusColor(r.status)}`}>
                      {r.status.replace(/_/g, " ")}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-foreground">{r.book_title || "—"}</td>
                  <td className="px-4 py-3 text-muted-foreground">
                    <div className="text-foreground">{r.lead_author_name || "—"}</div>
                    <div className="text-xs">{r.lead_author_email || ""}</div>
                  </td>
                  <td className="px-4 py-3 text-center text-muted-foreground">{r.author_count ?? "—"}</td>
