...
const AccordionTrigger = React.forwardRef<React.ElementRef<typeof AccordionPrimitive.Trigger>, React.ComponentPropsWithoutRef<typeof AccordionPrimitive.Trigger>>(({
  ...
  ...props
    ...
    <AccordionPrimitive.Trigger ref={ref} className={cn("flex flex-1 items-center justify-between transition-all font-nav [&[data-state=open]>svg]:rotate-180 text-left text-[#333333] hover:text-accent hover:no-underline py-5 text-[17px] leading-[1.6] [&>svg]:hidden font-bold", className)} {...props}>
      {children}
