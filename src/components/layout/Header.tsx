      ...
      <nav className="container-wide">
        <div className="flex items-center justify-between h-24">
          ...
          <div className="hidden lg:flex items-center gap-8 xl:gap-12">
            ...
            {navigation.slice(3).map(item => <Link key={item.name} to={item.href} className={cn("xl:text-lg font-medium transition-colors duration-200 relative py-2 text-black text-lg", location.pathname === item.href ? "text-accent after:absolute after:bottom-0 after:left-0 after:w-full after:h-0.5 after:bg-accent" : "text-foreground")}>
                {item.name}
              </Link>)}
          </div>
