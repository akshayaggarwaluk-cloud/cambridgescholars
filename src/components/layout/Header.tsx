      ...
      <nav className="container-wide">
        <div className="flex items-center justify-between h-24">
          ...
          <div className="hidden lg:flex items-center gap-8 xl:gap-12">
            {navigation.slice(0, 3).map(item => <Link key={item.name} to={item.href} className={cn("xl:text-lg font-medium transition-colors duration-200 hover:text-[#E4573D] relative py-2 text-lg", location.pathname === item.href ? "text-accent after:absolute after:bottom-0 after:left-0 after:w-full after:h-0.5 after:bg-accent" : "text-foreground")}>
                {item.name}
              </Link>)}
            ...
            <div className="relative" onMouseEnter={() => setIsPublishDropdownOpen(true)} onMouseLeave={() => setIsPublishDropdownOpen(false)}>
              <Link to="/publish-a-book" className={cn("text-base xl:text-lg font-medium transition-colors duration-200 hover:text-accent flex items-center gap-1.5 relative py-2", location.pathname === "/publish-a-book" || publishDropdownItems.some(item => location.pathname === item.href) ? "text-accent after:absolute after:bottom-0 after:left-0 after:w-full after:h-0.5 after:bg-accent" : "text-foreground")}>
                Publish a Book
