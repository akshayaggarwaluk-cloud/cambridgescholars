    ...
    >
      <nav className="container-wide">
        <div className="flex items-center justify-between h-24">
          ...
          {/* Desktop Navigation */}
          <div className="hidden lg:flex items-center gap-8 xl:gap-12 font-nav">
            ...
            >
              <Link
                to="/publish-a-book"
                className={cn(
                  "text-lg font-medium flex items-center gap-1.5 py-2 hover:text-[#E4573D]",
                  location.pathname === "/publish-a-book" ||
                    publishDropdownItems.some((i) => location.pathname === i.href)
                    ? "text-[#E4573D]"
                    : "text-foreground",
                )}
              >
                  ...
                  <div className="bg-background border rounded-lg shadow-xl py-2 w-60">
                    {publishDropdownItems.map((item) => (
                      <Link
                        ...
                        to={item.href}
                        className={cn(
                          "block px-5 py-2.5 hover:bg-secondary",
                          location.pathname === item.href ? "text-[#E4573D] font-medium" : "text-foreground",
                        )}
                      >
                        {item.name}
                      </Link>
            ...
            {navigation.slice(3).map((item) => (
              <Link
                ...
                to={item.href}
                className={cn(
                  "text-lg font-medium py-2 hover:text-[#E4573D]",
                  location.pathname === item.href ? "text-[#E4573D]" : "text-foreground",
                )}
              >
                {item.name}
          ...
          <div className="hidden lg:flex items-center gap-2">
            <Button
              variant="ghost"
              size="icon"
              className="h-11 w-11 hover:text-[#E4573D]"
            ...
            <Link to="/wishlist" className="relative">
              <Button variant="ghost" size="icon" className="hover:text-[#E4573D]">
                ...
                {wishlistCount > 0 && (
                  <span className="absolute -top-1 -right-1 h-5 w-5 rounded-full bg-[#E4573D] text-white text-xs flex items-center justify-center">
                    {wishlistCount}
                  </span>
                )}
              </Button>
