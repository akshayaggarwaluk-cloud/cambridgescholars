...
export function FeaturedBooksSection() {
  ...
  if (featuredBooks.length === 0) {
    return (
      <section className="py-20 bg-[#f4f3ec] overflow-hidden md:py-[20px]">
        <div className="container-wide">
           <p className="font-baskerville text-accent uppercase tracking-[0.3em] mb-4 text-lg font-sans font-normal">Featured Book</p>
      ...
      <div className="container-wide">
        <div className="relative grid md:grid-cols-[1.4fr_0.6fr] gap-8 items-center min-h-[500px]">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                 <p className="font-baskerville text-[#e4573d] uppercase tracking-[0.3em] text-lg font-sans font-normal">Featured Book</p>
                  <h2 className="text-lg font-sans font-normal font-baskerville text-3xl md:text-4xl font-normal leading-[1.15] text-foreground lg:text-4xl">{book.title}</h2>
                {book.description && (
                   <p className="font-sans text-lg font-normal font-nav text-muted-foreground leading-relaxed text-base italic font-normal">"{book.description}"</p>
                )}
                {book.reviewer && (
                 ...
                 <Button asChild variant="premium" className="px-16 py-3.5 text-sm tracking-[0.2em] rounded-none uppercase">
          ...
          <div className="order-1 md:order-2 flex justify-center md:justify-start md:pl-8">
            <AnimatePresence mode="wait">
              <motion.img key={activeIndex} src={book.image} alt={book.title} className="w-56 md:w-64 lg:w-[300px] xl:w-[340px]" style={{ filter: "drop-shadow(18px 22px 18px rgba(0,0,0,0.35))" }} initial={{ opacity: 0, scale: 0.95, x: 40 }} animate={{ opacity: 1, scale: 1, x: 0 }} exit={{ opacity: 0, scale: 0.95, x: -40 }} transition={{ duration: 0.8, ease: [0.25, 0.4, 0.25, 1] }} />
            </AnimatePresence>
          </div>
        </div>
