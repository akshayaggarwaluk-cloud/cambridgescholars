    ...
    >
      <div className="container-wide">
        <div className="relative grid md:grid-cols-[1.4fr_0.6fr] gap-8 items-center min-h-[500px]">
          ...
          <div className="order-1 md:order-2 flex justify-center md:justify-end md:-mr-16 lg:-mr-32 xl:-mr-48">
            <AnimatePresence mode="wait">
              <motion.img key={activeIndex} src={book.image} alt={book.title} className="w-56 md:w-72 lg:w-[360px] xl:w-[420px]" style={{ filter: "drop-shadow(18px 22px 18px rgba(0,0,0,0.35))" }} initial={{ opacity: 0, scale: 0.95, x: 40 }} animate={{ opacity: 1, scale: 1, x: 0 }} exit={{ opacity: 0, scale: 0.95, x: -40 }} transition={{ duration: 0.8, ease: [0.25, 0.4, 0.25, 1] }} />
            </AnimatePresence>
          </div>
        </div>
